 ...
 */
public class ConclusionOfMeetAction extends DispatchAction {
	/*
	 * Generated Methods
	 */
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		ConclusionOfMeetForm conofmeetForm = (ConclusionOfMeetForm) form;
		boolean result = false;
		String loginName = (String)request.getSession().getAttribute("UserName");
		DBTools dbTools = new DBTools();
	
		String workername = conofmeetForm.getWorkername();
		if(workername == null || workername.equals(""))
			workername = loginName;
		String time = conofmeetForm.getTime();		
		if(time == null || time.equals(""))
			time = SystemShare.GetNowTime("yyyy-MM-dd");
		String content = conofmeetForm.getContent();
		String reportID = conofmeetForm.getReportID();
		String id = conofmeetForm.getId();
		String attachName = (String)request.getSession().getAttribute("EventAttachName");
		if(attachName != null && !attachName.equals(""))
		{
			attachName = "cm" + reportID + "/" + attachName;
		...
		if(id != null && !id.equals(""))
		{
			...
			{
				sql = "update TB_CONOFMEET set REPORTID=?, WORKERNAME=?, TIME=?, CONTENT=? where ID=?";
				params = new String[]{reportID,workername, time, content, id};
			//	sql = "update TB_DEPTADVICE set DEPT='" + dept + "', TIME='" + time + "', ADVICE='"  + advice + "',EXPERTADVICE='" + expertAdvice + "',ISFK='1' where ID=" + id;
			}
			else
			{
				sql = "update TB_CONOFMEET set REPORTID=?, WORKERNAME=?, TIME=?, CONTENT=?, ATTACHNAME=? where ID=?";
				params =  new String[]{reportID,workername, time, content, attachName, id};
		...
		//	sql = "insert into TB_DEPTADVICE(REPORTID,DEPT,TIME,ADVICE,EXPERTADVICE,ISFK,ATTACHNAME,ISLETTER) values('" + reportID + "','" + dept + "','" + time + "','" + advice + "','" + expertAdvice + "','1','" + attachName + "', '0')";
			sql = "insert into TB_CONOFMEET(REPORTID,WORKERNAME,TIME,CONTENT,ATTACHNAME) values(?,?,?,?,?)";
			params = new String[]{reportID, workername, time, content, attachName};
		}
		String filePath = request.getSession().getServletContext().getRealPath("/")+"/attachment/cm/";
		...
		String relDirectory = "attachment/cm/" + reportID;
		...
		if(result)
		{
			String describe = time + "," + createName + "   编辑会议结论";
			//插入处理过程到数据库中
			result = dbTools.InsertHandleProcess(reportID, createName, SystemConstant.HP_CONOFMEET, SystemConstant.SS_HANDLEDECIDE, SystemConstant.LCT_HYJL, describe);
			
			//写入日志文件
			dbTools.insertLogInfo(createName, SystemConstant.LOG_CONOFMEET, "编辑会议结论，事件编号为：" + reportID, request.getRemoteAddr());
		...
		if(result)
		{
			...
			json.put("navTabId", "addConOfMeet");
		}
		else
		{
